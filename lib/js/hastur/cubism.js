...
cubism_contextPrototype.hastur = function(host) {
  if (!arguments.length) host = "";
  var source = {},
      context = this;

  source.metric = function(hosts_in, names_in, expression) {
    var hosts = [].concat(hosts_in);
    var names = [].concat(names_in);
    return context.metric(function(start, stop, step, callback) {
      d3.json(host + "/api/node/" + hosts.join() + "/name/" + names.join() + "/value"
          + "?fun=" + encodeURIComponent("format_cubism(" + expression + ")")
          + "&start=" + (start * 1000)
          + "&end=" + (stop * 1000), function(data) {
